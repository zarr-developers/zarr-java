...
public class ZarrTest {
  ...
  public void testReadFromZarrita(String codec, String codecParam) throws IOException, ZarrException, InterruptedException {
    ...
    ProcessBuilder pb = new ProcessBuilder(command, PYTHON_TEST_PATH.resolve("zarrita_write.py")
  ...
  public void testWriteReadWithZarrita(String codec, String codecParam) throws Exception {
    ...
    ProcessBuilder pb = new ProcessBuilder(command, PYTHON_TEST_PATH.resolve("zarrita_read.py")
        .toString(), codec, codecParam, TESTOUTPUT.toString());
    Process process = pb.start();
  ...
  public void testHttpStore() throws IOException, ZarrException {
    ...
    dev.zarr.zarrjava.v2.Array v2Array = dev.zarr.zarrjava.v2.Array.open(httpStore.resolve("l4_sample", "color", "1"));
    Array v3Array = Array.open(httpStore.resolve("zarr_v3", "l4_sample", "color", "1"));

    System.out.println(v2Array.metadata.fillValue);
    System.out.println(v2Array.metadata.dataType);
    System.out.println(v2Array.metadata.chunks[0]);
    System.out.println(v2Array.metadata.filters);
    System.out.println(v2Array.metadata.compressor);
    System.out.println(v2Array.metadata.order);
    System.out.println(v2Array.metadata.dimensionSeparator);
    System.out.println(v3Array);
    for (Codec codec : ((ShardingIndexedCodec) v3Array.metadata.codecs[0]).configuration.codecs) {
