...
public interface Plate {
    ...
    static Plate open(StoreHandle storeHandle) throws IOException, ZarrException {
        ...
        if (zarrJson.exists()) {
            ...
            if (attrs != null && attrs.has("ome") && attrs.get("ome").has("plate")) {
                ...
                if (version.startsWith("0.6")) {
                    return dev.zarr.omezarr.v0_6.Plate.openPlate(storeHandle);
                }
                return dev.zarr.omezarr.v0_5.Plate.openPlate(storeHandle);
            }
        }
        ...
        if (zattrs.exists()) {
            ...
            com.fasterxml.jackson.databind.JsonNode root = mapper.readTree(bytes);
            if (root.has("plate")) {
                return dev.zarr.omezarr.v0_4.Plate.openPlate(storeHandle);
            }
        }
