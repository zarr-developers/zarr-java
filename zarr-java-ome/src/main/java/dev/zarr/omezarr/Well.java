...
public interface Well {
    ...
    static Well open(StoreHandle storeHandle) throws IOException, ZarrException {
        ...
        if (zarrJson.exists()) {
            ...
            if (attrs != null && attrs.has("ome") && attrs.get("ome").has("well")) {
                ...
                if (version.startsWith("0.6")) {
                    return dev.zarr.omezarr.v0_6.Well.openWell(storeHandle);
                }
                return dev.zarr.omezarr.v0_5.Well.openWell(storeHandle);
            }
        }
        ...
        if (zattrs.exists()) {
            ...
            com.fasterxml.jackson.databind.JsonNode root = mapper.readTree(bytes);
            if (root.has("well")) {
                return dev.zarr.omezarr.v0_4.Well.openWell(storeHandle);
            }
        }
